...
from raindropio_mcp.models import BookmarkUpdate, HighlightUpdate
...
class TestAPIErrorScenarios:
    ...
    async def test_404_not_found_error(self) -> None:
        ...
        client = RaindropClient(settings=RaindropSettings(token="test_token_1234567890abcdefghijklmnopqr"))
    ...
    async def test_401_unauthorized_error(self) -> None:
        ...
        client = RaindropClient(settings=RaindropSettings(token="invalid_token_1234567890abcdefghijklmnopqr"))
...
class TestNetworkErrorScenarios:
    ...
    async def test_connection_timeout(self) -> None:
        ...
        with patch.object(client, "_client") as mock_client:
            mock_client.request = AsyncMock(side_effect=httpx.ConnectTimeout("Connection timeout"))
    ...
    async def test_network_error(self) -> None:
        ...
        with patch.object(client, "_client") as mock_client:
            mock_client.request = AsyncMock(side_effect=httpx.NetworkError("Network unreachable"))
...
class TestRetryLogicScenarios:
    ...
    async def test_retry_on_429(self) -> None:
        """Test retry logic on rate limit (mocked at get_json level)."""
        client = RaindropClient(settings=RaindropSettings(token="test_token_1234567890abcdefghijklmnopqr"))
    ...
    async def test_retry_on_500(self) -> None:
        """Test retry logic on server error (mocked at get_json level)."""
        client = RaindropClient(settings=RaindropSettings(token="test_token_1234567890abcdefghijklmnopqr"))
    ...
    async def test_retry_exhausted(self) -> None:
        """Test retry exhaustion (mocked at get_json level)."""
        client = RaindropClient(settings=RaindropSettings(token="test_token_1234567890abcdefghijklmnopqr"))
        ...
        with patch.object(
            ...
            with pytest.raises(ResourceNotFoundError):
                await client.update_bookmark(
                    999999,
                    BookmarkUpdate(link="https://example.com", title="Updated Title"),
                )
        ...
        with patch.object(
            ...
            with pytest.raises(ResourceNotFoundError):
                await client.update_highlight(
                    999999,
                    HighlightUpdate(text="Updated text"),
                )
...
class TestImportExportErrorScenarios:
    ...
    async def test_import_invalid_data(self) -> None:
        ...
        from raindropio_mcp.models.import_export_payloads import ImportSource
        ...
        with patch.object(
            ...
            with pytest.raises(APIError) as exc_info:
                await client.import_bookmarks(
                    ImportSource(format="html", source="browser"),
                )
    ...
    async def test_export_collection_not_found(self) -> None:
        ...
        from raindropio_mcp.models.import_export_payloads import ExportFormat
